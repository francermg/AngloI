...
begin
  ...
  DM.Alunos.Open;
  DM.Bloquetos.Open;
// if Principal.Ano.Caption<>'2004' then
...
var cont_adic,cont_alt,i, parc_ini, parc_fin, mesc  : integer;
...
Begin
       ...
       UltimaParcela.Open;
       parc_ini :=  UltimaParcelaTOTAL.Value + 1;    //Coloca a ˙ltima parcela
     ...
     Begin
       ...
       if ((DM.AlunosBLOQUETO_1SEM.Value = 1) and (dm.AlunosBOLSA.Value <> 100)) then  inc(cont_adic); //para contar quantos boletos foram adicionados
       if ((DM.AlunosBLOQUETO_1SEM.Value = 2) and (dm.AlunosBOLSA.Value <> 100)) then inc(cont_alt);


       if ((dm.AlunosBLOQUETO_1SEM.Value = 1) and (dm.AlunosBOLSA.Value <> 100)) then
       Begin
          if (dm.AlunosBLOQUETO_1SEM.Value = 2) then
          begin
              copiaBoleto;
              Delete_Boleto.Close;
              Delete_Boleto.Params[0].Value := dm.AlunosCODIGO.Value;
              Delete_Boleto.Open;
              UltimaParcela.Close;
              UltimaParcela.Params[0].Value := dm.AlunosCODIGO.Value;
              UltimaParcela.Open;
              parc_ini :=  UltimaParcelaTOTAL.Value + 1;    //Coloca a ˙ltima parcela
                ...
                begin
                    ...
                    d_dia := (DaysPerMonth(agoraano, mesc));
                    DM.BloquetosVENCIMENTO.Value := StrToDate(IntToStr(d_dia)+'/'+IntToStr(mesc)+'/'+IntToStr(agoraano));
                    ...
                    //dm.BloquetosTP.Value := ((parc_fin - parc_ini) + 1);
                    ...
                    if (mesc = 2) then
                      DM.BloquetosDATA_LIMITE.Value := EncodeDate(agoraano, mesc,25) //Data limite para pagamento de pontualidade do mÍs de fevereiro
                      else
