...
var
  Doc_AtaRecQR: TDoc_AtaRecQR;

implementation

uses UDM, UcadParametros, UDoc_Ata, UMenu, UDoc_AtaRec, UDM2;

{$R *.DFM}

procedure TDoc_AtaRecQR.TitleBand1BeforePrint(
...
begin
  ...
  TxtSecretaria.Caption := StringReplace(TxtSecretaria.Caption, '{SECRETARIA}', Doc_AtaRec.Secretaria.Text, [rfReplaceAll]);
  DecodeDate(Doc_AtaRec.DataDoc.Date,ano,mes,dia);
  if dia = 1 then diaX := '1º' else
   diaX := FormatFloat('00',dia);
  Tit1.Caption := StringReplace(Tit1.Caption, '{DIA}', diaX, [rfReplaceAll]);
  Tit1.Caption := StringReplace(Tit1.Caption, '{MES}', DM.MesExtS(Doc_AtaRec.DataDoc.Date), [rfReplaceAll]);
  ...
  if Doc_AtaRec.RadioGroup1.ItemIndex = 0 Then
  Tit1.Caption := StringReplace(Tit1.Caption, '{TIPO}', 'Reclassificação', [rfReplaceAll])
  Else
      Tit1.Caption := StringReplace(Tit1.Caption, '{TIPO}', 'Adaptação', [rfReplaceAll]);
  AnoX.Caption := 'Ano Letivo de '+Principal.Ano.Caption;
  DetNota.Color := DM.ParametrosCOR_ZEBRADO.Value;
end;

procedure TDoc_AtaRecQR.DetNotaBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TDoc_AtaRecQR.M01Print(sender: TObject; var Value: String);
begin
