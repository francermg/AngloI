...
//Inicio Detalhe ******************************************************************************************************
    ...
    begin
      ...
      while (not dm.BloquetosQ.Eof) do
             if ((dm.BloquetosQPAGAMENTO.IsNull) and (not dm.BloquetosQSACADO_CPF.IsNull)) then  // and (dm.AlunosCODIGO.Value = 21203) se o pagamento ainda não foi realizado
            begin
      ...
      //            ShowMessage ('Pare');
                ...
                aux := aux + desconto;
                //180 a 192 Valor do desconto  coloque o campo desconto que é o desconto para pagar até o dia 10
                //193 a 205 Filler
               ...
               showmessage('dndnjdnjndj');
                ...
                // desativei o cep do pagador e coloquei o da instituição para ficar padrão w_cep;
                //315 a 319 Código do pagador na cooperativa beneficiário
                //320 a 325 Filler
                //326 a 326 Filler
  ...
  ShowMessage ('Não há boletos a serem registrados.');
end;
        ...
        if DM.Bloquetos.Locate('NOSSO_NUMERO', VarArrayOf([nosso_numerob]), [loPartialKey]) then
          begin
            ...
            if Copy(Linha,319,10) = '3400000000' then
              dm.BloquetosMOTIVO.Value := 'Valor do abatimento maior/igual ao valor do título';
            if Copy(Linha,319,10) = '4800000000' then
               dm.BloquetosMOTIVO.Value := 'CEP irregular';
            if Copy(Linha,319,10) = '4600000000' then
               dm.BloquetosMOTIVO.Value := 'Tipo/número de instrução do pagador inválidos';
               dm.BloquetosST.Value := 'R';
           if Copy(Linha,319,10) = '2000000000' then
            begin
               dm.Bloquetos.Edit;
               dm.BloquetosMOTIVO.Value := 'Valor do titulo inválido/Bolsa 100%';
               dm.BloquetosST.Value := 'B';
            end;
           dm.Bloquetos.Post;
          end;
...
{            dm.ibRetornoMotivo.Insert;
            ...
            if Copy(Linha,319,10) = '2000000000' then
               dm.ibRetornoMotivoMOTIVO.Value := 'Valor do titulo inválido/Bolsa 100%';
