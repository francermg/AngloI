...
begin
  SerX := IntToStr(Serie);
  if MessageDlg('Distribui da '+SerX+'ª Série nas turmas?',mtConfirmation,mbOKCancel,0) = mrOK then begin
    ...
    Gauge2.Progress := 0;
    Gauge2.Show;
    DM.Turmas.SelectSQL.Strings[2] := 'where serie = '+SerX+' order by TURMA'; //seleciona as turmas da serie em que está distribuindo
    DM.Turmas.Open;
    while not DM.Turmas.Eof do begin    //enquanto não teminar a tabela de turmas
      DM.Turmas.Edit;
    ...
    DM.IBTr_ANGLO.CommitRetaining;
    Gauge2.Hide;
    ShowMessage('Alunos da '+SerX+'ª Série distribuídos!');
  end;
end;
...
begin
  ...
  TurX := ' ';
  Num := 0;
  if MessageDlg('Reorganiza Números da '+SerX+'ª Série ?',mtConfirmation,mbOKCancel,0) = mrOK then begin
    DM.Alunos.SelectSQL.Strings[2] := 'where SERIE = '+SerX+
    ...
    DM.IBTr_ANGLO.CommitRetaining;
    Gauge3.Hide;
    ShowMessage('Números dos alunos da '+SerX+'ª Série reorganizados!');
  end;
end;
