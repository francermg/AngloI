...
type
  TCad_Alunos = class(TForm)
    ...
    Edit3: TEdit;
    Label119: TLabel;
    dbmmoObsTelefone: TDBMemo;
    btncomunicado: TButton;
    ...
    procedure btncomunicadoClick(Sender: TObject);
  private
...
procedure TCad_Alunos.BreprocessaClick(Sender: TObject);
begin
if (dm.Bloquetos.State in [dsEdit, dsInsert]) Then  dm.Bloquetos.Post;
BCopia;
RBoletos;
AtualizaSitFin2;
end;
...
begin
   Try
      ...
      if (RadioGroup1.ItemIndex = 1) then
            Doc_historico_2022.RLLabel1.Caption := 'Transferência';
...
procedure TCad_Alunos.btncomunicadoClick(Sender: TObject);
var w_codigo :string;
begin
  try
    w_codigo := dm.AlunosCODIGO.AsString;
    dm.Alunos.Close;
      DM.Alunos.SelectSQL.Strings[2] := 'where codigo = :bcodigo';
      DM.Alunos.ParamByName('bcodigo').AsString := w_codigo;
    dm.Alunos.Open;
    comunicadoq.preview;
  finally
    dm.Alunos.Close;
