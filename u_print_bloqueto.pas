...
//  RLLabel18.Visible := True;
  ...
  if (((principal.Ano.Caption = '2012') or (principal.Ano.Caption = '2013') or
       ...
       (Principal.Ano.Caption = '2020') or (Principal.Ano.Caption = '2021'))) then

    rlblVencimento1.Caption := dm.BloquetosQVENCIMENTO.AsString;
    rlblVencimento2.Caption := dm.BloquetosQVENCIMENTO.AsString;

     if ((dm.AlunosSTATUS.Value = 'P') or (dm.BloquetosQParcelaX.Value = '01ª')) Then
       Begin
        MENSAGEM1.Caption := '  PAGAMENTO REFERENTE A PRÉ-MATRÍCULA.';
        MENSAGEM2.Caption := '  A matricula será efetuada após a confirmação do pagamento deste.  COLÉGIO UNIGRAN';
